...
| Name | Description |
|---|---|
| [`sl-overall-review`](skills/sl-overall-review/) | Multi-perspective code review of the current branch against a base you pick. Selects one of 12 profiles (universal / bug-fix / feature / refactor / research / performance / security / migration / algorithm / messaging / skill / docs) and runs the matching reviewers in parallel. The `skill` profile turns the review on **prompt artifacts themselves** — a `SKILL.md` and its bundle, subagent and slash-command definitions, `CLAUDE.md` — grading them against context-engineering principles for Claude 5 generation models. Full docs and reviewer breakdown: [skills/sl-overall-review/README.md](skills/sl-overall-review/README.md). |
